...
void create()
{
	set_name("何泰冲", ({ "he taichong", "he", "taichong" }));
...
{
	command("say 哼，如我何泰冲内力尚在，也由不得你猖狂。");
	return 0;
}
...
{
	command("heng");
	command("say 我何泰冲被你们用诡计擒住，早就没想过要继续存活，要杀就动手吧。");
...
int begin()
{
	    ...
	    (string)weapon->query("skill_type") != "sword")
	{
		...
		return notify_fail("看起来何泰冲并不想跟你较量。\n");
	}

	command("heng");
	command("say 进招吧！");
	message_vision(HIY "$N话音未落，单手一抖，手中竹侍对准$n" HIY
