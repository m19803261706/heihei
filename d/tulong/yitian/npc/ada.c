...
*/
	...
	if (sword)
	{
		...
		message_vision(CYN "突然赵闵喝道：“慢着！拿着倚天剑和$n" CYN
				   "斗！”"NOR"\n",
				   obj, this_player());
...
void unconcious()
{
	...
	if (objectp(query("anti")))
	{
		...
		message_vision(CYN "\n赵闵一张俏脸顿时惨白，叹了口气，说道：“很好，这倚天剑你就拿去吧！”"NOR"\n",
				   this_object());

		message_vision(CYN "赵闵对着$n" CYN 
				   "嫣然一笑，随即轻轻伸了伸腰，轻声说道：“鹿先生，鹤先生，我累了。”"NOR"\n",
				   this_object(), me);

		message_vision(HIR "只见赵闵身后闪出两个人影，相互使了个眼神，二话不说，对着$n" HIR 
				   "直扑过来。”"NOR"\n",
				   this_object(), me);
