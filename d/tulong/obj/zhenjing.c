...
void create()
{
	set_name(HIY "《" HIW "玄阴真经" HIY "》" NOR, ({ "jiuyin zhenjing", "zhenjing"}));
	set_weight(500);
	if (clonep())
		...
		set("no_put", 1);
		set("long", HIW
	"这是一本用薄纸写成的书。书面上写着《玄阴真经》。书皮泛黄，看来已经保存很久了。\n"
	"真经里记载了催心掌、玄阴炼骨爪、易筋锻骨和玄阴步，你可以试着研究研究。"NOR"\n", );
	}
	set("action_list", ([
		"研究" : "yanjiu2",
		"停止研究": "stopread",
...
int do_du(string arg)
{
	...
	{
		write(HIW "你可以从玄阴真经里研究到易筋锻骨（yijin-duangu），玄阴步（shexing-lifan），\n"
			  "催心掌（cuixin-zhang）以及玄阴炼骨爪（jiuyin-baiguzhao）。"NOR"\n",);
		return 1;
	}
	      ...
	      if (! me->query_skill("literate", 1))
	{
		...
		if (me->query("character") == "狡黠多变" && (int)me->query("yijin-duangu", 1) > 199)
			return notify_fail("这玄阴真经中所记载的易筋锻骨正大恢弘，气度俨然，依照"
				   "你狡黠多变的天性似乎只能学到这种程度。\n");
	...
	if (skill == "shexing-lifan")
	{
		...
		write("你仔细研读玄阴真经，对真经里所记载的玄阴步轻功颇有心得。\n");
	}
	else
	      ...
	      if (! me->query_skill("literate", 1))
	{
		...
		write("你仔细研读玄阴真经，对真经里所记载的催心掌颇有心得。\n");
	}
	else
	      ...
	      if (! me->query_skill("literate", 1))
	{
		...
		write("你仔细研读玄阴真经，对真经里所记载的玄阴炼骨爪颇有心得。\n");
	}
	else
	{
		write("真经里面并没有记录这些技能。\n");
