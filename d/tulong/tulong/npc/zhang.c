...
void create()
{
	set_name("张崔衫", ({ "zhang cuishan", "zhang" }));  
	set("long",
...
void biwu(object me)
{
	...
	message_vision(HIR "\n张崔衫说着缓步走到左首山峰前一堵大石壁前，吸一口气，猛地里双脚一撑，提身而起。"NOR"\n",this_object());
	remove_call_out("biwu2");
	call_out("biwu2",8,me);
}
void biwu2(object me)
{
	message_vision(HIR "\n张崔衫身形纵起丈余，跟着使出“梯云纵”绝技，右脚在山壁一撑，一借力，又纵起两丈。"NOR"\n",this_object());
	message_vision(HIR "\n手中判官笔看准石面，嗤嗤嗤几声，已写了一个“"NOR+HIY"武"NOR+HIR"”字。"NOR"\n",this_object());
	message_vision(HIR "\n张崔衫一个字写完，身子便要落下，突然左手挥出，银钩在握，倏地一翻，钩住了石壁的缝隙。"NOR"\n",this_object());
...
{
	message_vision(HIR "\n张崔衫两个字写罢，跟着又写“"NOR+HIY"至"NOR+HIR"”字，“"NOR+HIY"尊"NOR+HIR"”字。越写越快，但见石屑纷纷而下。"NOR"\n", this_object());
	...
	message_vision(HIR "\n张崔衫写到“锋”字的最后一笔，银钩在石壁上一撑，翻身落地，轻轻巧巧的落在殷素娘身旁。"NOR"\n", this_object());
...
void arrive(object me)
{
	...
	message_vision(CYN "张崔衫抱拳说道：咱们就此别过，后会有期。"NOR"\n",me,this_object());
...
int do_get(string arg)
{
	...
	if(what == "tulong blade"||what == "blade"||what == "dao"||what == "tulong" || where =="ding")
       {
	      message_vision(CYN "$N说道：在下武当张崔衫，这厢有理了！"NOR"\n",this_object());
