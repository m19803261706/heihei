...
void create()
{
	set_name("鲁乾", ({ "lu kun","lu","kun", "man" }));
	set("title", "襄阳武馆大弟子");
	set("gender", "男性");
	set("age", 35);  
	set("long", "他是万巽山的大弟子，也是馆主最信任的徒弟，长得赳赳昂昂。\n");
...
string ask_job()
{
	...
	tell_object(ob, CYN"鲁乾说道：「"+str +"」"NOR"\n");
	...
	if( arg=="ok" && ob->query_temp("mark/完了")){
		...
		if(ob->query_temp("wg/lujob") < 5) {
			...
			if(ob->query_temp("jobover") != 5 && ob->query_temp("need_answer")) {
				tell_object(ob, CYN"鲁乾说道：「听说狄天云兄弟还有点事情找你，你最好去看一下。」"NOR"\n");
...
void greeting(object ob)
{
	...
	if(ob->query_temp("mark/还了")
	 && ob->query_temp("job_name")){ 
		...
		tell_object(ob, CYN"鲁乾说道：「"+ob->name()+"，你工作做得不错，可以跟我覆命了。」"NOR"\n");
		return;
	}
	...
	&& (ob->query_temp("jobask") == 5 || ob->query("combat_exp") >= 400) ) {
		...
		tell_object(ob, CYN"鲁乾说道：「这位"+ RANK_D->query_respect(ob)+ "，我们这缺人手，你可以找我领工作来做。」"NOR"\n");
	}
}
