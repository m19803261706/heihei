...
int do_duihuan(string arg)
{
	...
	ob->move(me);
	me->save();
	tell_object(me,"你从千机客那里兑换了一"+ob->query("unit")+ob->query("name")+"。\n");
	return 1;
}
...
string ask_me()
{
	...
	me = this_player();
	if (PK_D->is_pking())
		return "现在武林竞技大赛正在举行呢，你想干什么？";

	if (PK_D->is_ready())
		return "很好，现在武林竞技大赛正要开始，你要报名么(sign)？";

	return "武林竞技大赛？你慢慢等着吧，还没到开始时间呢。";
}
...
int do_join()
{
	...
	if (PK_D->is_pking())
		return notify_fail("千机客眼睛一弹，喝道：“现在里面正比赛呢，你这些人等等吧。”\n");

	if (PK_D->is_ready())
		return notify_fail("千机客懒懒道：“要想正式比赛就进去，其他人先等着。”\n");

	message_vision("$N赔笑对$n道：“我进去溜达溜达，放行可好？”\n",this_player(), this_object());

	if (PK_D->join_competition(this_player()))
	{
		message("vision", this_player()->name() + "一溜小跑进了竞技场。\n",
			environment());
		return 1;
	}
