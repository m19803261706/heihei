...
void create()
{
	set_name("任横行", ({ "ren woxing", "ren","woxing" }));
...
招，再说，我也未必能胜得了你。”\n\n", me, ob);
	...
	say(HIR"任横行大喝一声：“恒山剑法！”"NOR);
	COMBAT_D->do_attack(ob, me, query_temp("weapon"));
	if( (int)me->query("qi") < 0 || !present(me, environment()))
	{
		say("任横行叹了口气，说道：“还是没过八招。”\n");
		reset_skill();
		return 1;
	}
	...
	obj -> move(me);
	say("任横行说道：“这位" + RANK_D->query_respect(me)+"不错，我就送你一块黑木令吧。”\n");
	reset_skill();
	return 1;
}
void attempt_apprentice(object ob)
{
	...
	if (ob->query("family/master_id") != "xiang wentian") {
		tell_object(ob, "我一生只收项问天推荐过来的弟子，可惜你不是。\n");
		return;
	}
