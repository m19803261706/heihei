...
void create()
{
    set_name("东方教主", ({"dongfang bubai", "dongfang", "bubai"}));
	...
	set("inquiry", ([
		...
		"#31-葵阳宝录2"  : (: ask_baodian() :),
         "杨莲廷"   : "你敢直称我爱人的名字，想找死呀？ \n",
         "葵花上册" : "如果你能帮我取得葵阳宝录上册，我一定重重谢你。\n",
	]) );
	set("no_teach", ([
...
string ask_baodian()
{
   ...
   if (me->query_skill("kuihua-shengong", 1) < 1 && me->query_skill("pixie-jian", 1) < 180)
      return RANK_D->query_respect(me) + "的胜邪剑法好像练得不够火候？";

   ob = new("/clone/book/kuihuabaodian");
   ob->move(me);
   message_vision("$N在东方教主那里拿到一本"+ob->name()+NOR"。\n",me);
...
void attempt_apprentice(object ob)
{
   ...
   if (ob->query_skill("xixing-dafa", 1) > 1 || ob->query_skill("tianmo-jian", 1) > 1) {
         tell_object(ob, "你已经学会吸功大法或是天魔剑法，我怎能收你？\n");
         return;
   }
   if (ob->query("family/master_id") == "ren woxing") {
         tell_object(ob, "你个臭不要脸的，任横行派你来做卧底的吗？！\n");
         return;
   }
