...
int ljshop(string arg)
{
 ...
 int i,yuanbao;
	...
	if (! arg)
	{
		str = ZJOBLONG"你目前有"+me->query("lunjian/pot")+"论剑积分，请选择您想兑换的道具：\n"ZJOBACTS2+ZJMENUF(4,4,8,32);
...
int jjsign()
{
	...
	if (me->query("combat_exp")<200000)
		return notify_fail("你经验尚浅，还是把功夫练好再来吧。\n");
...
private void auto_check()
{
	...
	if(!sizeof(rooms))
	{
		...
		if(sizeof(us))
		{
			...
			log_ufile(ob,"lunjian","轮空晋级，积分增加1。\n");
		}
		turn++;
		return;
	}
	for (i = 0; i < sizeof(rooms); i++)
	{
		...
		else if(sizeof(inv)==1)
		{
			...
			log_ufile(inv[0],"lunjian","胜利晋级，积分增加1。\n");
			check_win(inv[0]);
		}
	}
}
