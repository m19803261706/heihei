...
string look_jing()
{
	...
	if (me->query("score") < 1000)
		return "你看了半天，只看到自己的影子，没发现什么。（阅历不足1000）\n";

	if (me->query("combat_exp") < 500000)
		return "你觉得上面镌的经偈似乎有些不同（经验不足50万）。\n";

	// return "你发现上面镌的经偈光泽和别处不一样，难道可以掀(xian)开？\n";
	return "你发现上面镌的经偈光泽和别处不一样，难道可以掀开？\n" + popup_buton(({ "掀开:xian" }));
}
...
int do_xian(string arg)
{
	...
	if (! arg)
	{
		if (query("status") == 4)
		{
			return notify_fail("铜镜已经被掀开了，你还要掀什么？（当前解密已经被人完成，请等待系统刷新）\n");
		}

		// return notify_fail("你要掀什么字？\n");
		return notify_fail(popup_input("共有如下字：一切有为法如梦幻泡影如露亦如电当作如是观,如果要掀开字(输入字和顺序，例如：梦 1，就是掀开第一个梦子)，请输入要掀的字：", "xian"));
	}
		// return notify_fail("你要掀什么字？\n");

	if (objectp(ob = present("xuanji dashi", this_object())) &&
	    living(ob))
		return notify_fail("玄静一眼扫了过来，冷冰冰"
				   "的似要将你看穿，你哪敢乱动？\n");
