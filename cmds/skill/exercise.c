...
int main(object me, string arg)
{
	...
	tell_object(me,"你可以"ZJURL("cmds:halt")"站起来"NOR"停止打坐。\n");
...
void auto_dazuo(object me, int exercise_cost)
{
	...
	else
	{
		...
		me->force_me("exercise "+exercise_cost);
	}
}
...
int exercising(object me)
{
	...
	{
	case 1:
		write(YEL "感知到" HIR "『百会穴』" NOR YEL "涌动起一股温暖的气流，你引导它开始沿小周天运行......" NOR "\n");
		break;
	case 2:
		write(YEL "气流缓缓渗入" HIW "『印堂穴』" NOR YEL "，似乎在眉心处回旋，渐渐深入......" NOR "\n");
		break;
	case 3:
		write(YEL "你能感受到气流经过" HIG "『檀中穴』" NOR YEL "，如漩涡般旋转，随后继续它的旅程......" NOR "\n");
		break;
	case 4:
		write(YEL "你小心翼翼地引导这股暖流返回" HIR "『百会穴』" YEL "，身体随着气机的回归而微微颤动。" NOR "\n");
		break;
	default:
		finish = 1;
		break;
	...
	me->set_temp("pending/exercise", 0);
	tell_object(me, "你缓缓睁开双眼，感受着体内流转的内力。深深地吸了一口气，随着气息的流动，你站了起来。\n"
	+ HIW "你明显感觉到自己的内功有了新的精进：\n"
	+ HIR "【内力】" NOR " " HIG "+" + neili_temp + NOR + HIR "  【" + to_chinese(map_temp["force"]) + "】" NOR " " HIG "经验值 +" + skill_temp_point + NOR "\n");
	...
	else
	{
		...
		else if ((int)me->query_skill("linji-zhuang",1) == 300 &&(int)me->query("max_neili") == 1000)
		{
			...
			me->set("neili", (int)me->query("max_neili"));
			write("你只觉你的内息游遍了你全身，仿佛容入了天地之间，你已经融会贯通了临济十二庄的地字庄。\n");
			return 0;
		}
		else
		{
