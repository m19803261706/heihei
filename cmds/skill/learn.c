...
void auto_learn(object me, string teacher,string skill,int times)
{
	...
	else if(me->query("neili")<302 && me->query("max_neili")>400)
	{
		...
		if (present("huxin dan",me) && me->query("combat_exp")<2000000 &&!me->query_condition("medicine"))
		{
	...
	else
	{
		...
		me->force_me("learn "+teacher+" "+skill+" "+times);
		// call_out("auto_learn",1,me,teacher,skill,times);
	}
}
...
int main(object me, string arg)
{
	...
	object ob;
	mixed flag;
	int master_skill, my_skill, jing_cost, times, i,single_point,next_my_skill;
	int up_point = 0;
	int t = time();
	mapping learn_map;
	...
	if (i >= 2 && sscanf(args[i], "%d", times) && times)
		...
		return notify_fail(INPUTTXT("请输入学习次数（每次请教提升1级，请注意潜能消耗）：","learn "+arg+" $txt#")+"\n");
	skill = args[i--];
	if (args[i] == "about" && i >= 1)
		i--;
	...
	// 	return notify_fail("学习次数最少一次，最多也不能超过一百万次。\n");
	...
	if (mapp(me->query_learned()))
	{
		learn_map = copy(me->query_learned());
	}
	else
	{
		learn_map = ([]);
	...
	if (undefinedp(learn_map[skill]))
	{
		learn_map[skill] = 0;
	}

	// 最大请教等级不能超过200
	if ((me->query_skill(skill, 1) + times) >= 200)
	{
		write("你的「" + to_chinese(skill) + "」已经无法从" + ob->name() + "那里学到更多了(不能超过200)。\n");
		return 1;
	}
	...
	// 1点潜能提升的熟练度
	single_point = me->query_int() + my_skill / 4;
	single_point /= 4;
	single_point = 5;
	single_point *= 2;

	//目标等级是下一级
	next_my_skill = my_skill + 1;

	// 提升熟练度
	while (me->query_skill(skill, 1) < next_my_skill)
	{
		// 判断潜能是否足够
		if (me->query("potential") - me->query("learned_points") < 100)
		{
		...
		me->improve_skill(skill, single_point * 100);
		me->add("learned_points", 100);
		up_point += single_point * 100;
	}
	...
	if (times > 0)
	{
		call_out("auto_learn",2,me,teacher,skill,times);
	}
