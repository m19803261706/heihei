...
int main(object me, string arg)
{
	...
	if( sscanf(arg, "%s %s d", skill, perform) == 2 ) {			
		...
		tell_object(me, HIR "你删除了自创" + to_chinese(skill) + "绝招" + to_chinese(perform) + "(" + perform + ")。"NOR"\n");						
	...
	if (sizeof(me->query("create/" + skill)) >= 3)
		return notify_fail("你的" + to_chinese(skill) + "绝招数量已经到了极限，无法再自创新绝招。\n");
...
int inventing(object me)
{
	...
	if( random(10) == 1 ) {
		...
		me->set_short_desc(0);   
		me->delete_temp("create");
		tell_object(me, HIW "任何时候输入 ~q 则可终止自创绝招。"NOR"\n");
		tell_object(me, HIW "你正在为" + to_chinese(skill) + "自创新的绝招。"NOR"\n"
				"你可以为这个绝招选择创建以下类型"NOR"\n");		
...
int halt_inventing(object me)
{
	me->set_short_desc(0);
	me->delete_temp("create");
	tell_object(me, HIG "你中止了自创绝招。"NOR"\n");
...
void get_perform_model(string arg, object me, string base_skill, string skill)
{
	...
	tell_object(me, HIW "\n你正在为" + to_chinese(skill) + "自创出了" + model + "的绝招。"NOR"\n"
			"你可以为这个绝招取一个名字\n"
			HIY "格式：" HIC "<英文名> <中文名>"NOR"\n"
...
void get_perform_name(string arg, object me, string base_skill, string skill, string model)
{
	...
	tell_object(me, HIC "\n注意问题: $N 代表你 $n 代表敌人 $w 代表你的武器 "NOR"\n");
	tell_object(me, HIC "例如：(拳脚类) "NOR"\n");
	tell_object(me, HIM "$N" HIM "一声怒嚎，将战神心经提运极至，双拳顿时携着雷霆万钧之势猛贯向$n" HIM "。"NOR"\n");
	tell_object(me, HIC "例如：(武器类)"NOR"\n"); 
	tell_object(me, HIM "$N" HIM "手中$w寒气逼人，刀身撕裂空气，片片雪花飘落，刀未至气先及，$n" HIM "眉毛都蒙上了薄薄白霜！"NOR"\n");
	tell_object(me, HIC "请输入发招时描述：（长度不超过200个汉字）"NOR"\n");			 
...
void get_perform_msg1(object me, string base_skill, string skill, string model, string c_name, string e_name, string msg)
{
	...
	tell_object(me, HIC "\n注意问题: $N 代表你 $n 代表敌人 $w 代表你的武器 "NOR"\n");
	tell_object(me, HIC "例如：(拳脚类) "NOR"\n");
	tell_object(me, HIM "结果$n" HIM "闪避不及，$N" HIM "的拳力掌劲顿时透体而入，口中鲜血狂喷，连退数步。"NOR"\n");
	tell_object(me, HIC "例如：(武器类)"NOR"\n"); 
	tell_object(me, HIM "只见$n" HIM "一声惨叫，胸口给劈开一个巨大的口子，鲜血汹涌喷出！"NOR"\n");
	tell_object(me, HIC "请输入绝招打中时描述：（长度不超过100个汉字）"NOR"\n");    
...
void get_perform_msg2(object me, string base_skill, string skill, string model, string c_name, string e_name, string pfm_attack_msg, string msg)
{
	...
	tell_object(me, HIC "\nOK"NOR"\n");   
	tell_object(me, HIC "\n注意问题: $P 代表你 $p 代表敌人 $w 代表你的武器 "NOR"\n");
	tell_object(me, HIC "例如：(拳脚类) "NOR"\n");
	tell_object(me, HIM "可是$p" HIM "识破了$P" HIM "这一招，斜斜一跃避开。"NOR"\n");
	tell_object(me, HIC "例如：(武器类)"NOR"\n"); 
	tell_object(me, HIM "$p" HIM "见势不妙，抽身急退，险险避过$P" HIM "的这记杀招，尘土飞扬中，地上裂开了一道大口子！"NOR"\n");	
	tell_object(me, HIC "请输入绝招失误时描述：（长度不超过100个汉字）"NOR"\n");   
...
void get_perform_msg3(object me, string base_skill, string skill, string model, string c_name, string e_name, string pfm_attack_msg, string pfm_succeed_msg, string msg)
{
	...
	tell_object(me, HIW "\n你成功的为" + to_chinese(skill) + "新创的绝招设置以下相关信息。"NOR"\n"
			HIC "绝招类型：" NOR HIC + model + ""NOR"\n"
			HIC "中文名称：" NOR HIC + c_name + ""NOR"\n"
			HIC "英文名称：" NOR HIC + e_name + ""NOR"\n"
			HIC "发招描述：" NOR HIC + pfm_attack_msg + ""NOR"\n"
			HIC "打中描述：" NOR HIC + pfm_succeed_msg + ""NOR"\n"
			HIC "失误描述：" NOR HIC + pfm_fail_msg + ""NOR"\n"
			HIW "你确定吗？(Y/N)" NOR);
			  ...
			  string e_name, string pfm_attack_msg, string pfm_succeed_msg, string pfm_fail_msg)
{
	...
	if( arg == "y" || arg == "Y" ) {
		...
		tell_object(me, HIW "\n你为" + to_chinese(skill) + "自创出了新的绝招。"NOR"\n"
				"你可以重新选择为这个绝招选择创建以下类型"NOR"\n");      
		tell_object(me, "1> 创建一个为连招的绝招\n");
		tell_object(me, "2> 创建一个为单招的绝招\n");
		tell_object(me, "3> 创建一个为混合招的绝招\n");   
		tell_object(me, "3> 请重新选择1-3：");       
			  ...
			  string e_name, string pfm_attack_msg, string pfm_succeed_msg, string pfm_fail_msg)
{
	...
	write(HIC "绝招创建成功，以后请使用 " + HIY "perform " + e_name +  HIC " 使用该绝招。"NOR"\n" );
}
