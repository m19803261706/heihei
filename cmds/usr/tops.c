 ...
 string type;
	...
	if(!arg)
	{
		if (SECURITY_D->valid_grant(me, "(admin)"))
			write(ZJOBLONG"请选择排行类型：\n"ZJOBACTS2+ZJMENUF(4,4,9,30)+"修为榜:tops 修为"ZJSEP"容貌榜:tops 容貌"ZJSEP
				"财富榜:tops 财富"ZJSEP"杀手榜:tops 杀气"ZJSEP"师门榜:tops 师门"ZJSEP"积分榜:tops 积分\n");
		else
			write(ZJOBLONG"请选择排行类型：\n"ZJOBACTS2+ZJMENUF(4,4,9,30)+"修为榜:tops 修为"ZJSEP"容貌榜:tops 容貌"ZJSEP
	...
	if(type!="修为"&&type!="容貌"&&type!="财富"&&type!="杀气"&&type!="师门"&&type!="积分")
...
//			text += ({ GRN"第"+(i+1)+"名 - "+all_users[i]+NOR+ZJBR+YEL+"财富："+TOP_D->query_user(all_users[i]+"/gold")+NOR":look" });
	...
	else if(type=="积分")
	{
		...
		for(i=0;i<sizeof(all_users);i++)
		{
			text += ({ GRN"第"+(i+1)+"名 - "+TOP_D->get_name(all_users[i])+NOR+ZJBR+YEL+"积分："+TOP_D->query_user(all_users[i]+"/all_pay")+NOR":look "+all_users[i] });
//			text += ({ GRN"第"+(i+1)+"名 - "+all_users[i]+NOR+ZJBR+YEL+"积分："+TOP_D->query_user(all_users[i]+"/all_pay")+NOR":look "+all_users[i] });
		}
	}
	else if(type=="容貌")
	{
