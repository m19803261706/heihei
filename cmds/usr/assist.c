...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		if (objectp(ob = me->query_temp("quest/assist")))
			write("你现在正帮助" CYN + ob->name(1) + NOR "完成门派的任务。\n");
		else
	...
	tell_object(ob, YEL + me->name(1) + YEL "决定帮助你一同完成任务，你是否同意(right|refuse " + me->query("id") + ")？"NOR"\n");
...
int do_right(object me, object ob)
{
	...
	if (! pointerp(me->query_team()) || me->is_team_leader())
	{
		...
		{
			write("你这个队伍中的人太多了，没法邀请" + ob->name(1) + "加入，这样让他帮助就不方便了。\n");
			return 1;
		}
