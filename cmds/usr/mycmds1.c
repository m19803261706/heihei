...
int main(object me, string arg)
{
	...
	if(!arg)
	{
		...
		write(ZJBTSET"b1:查看"ZJBR"技能:skills"ZJSEP"b2:日常"ZJBR"练功:help liangong"ZJSEP"b3:内力"ZJBR"回血:exert recover"
			...
			ZJSEP"b7:内力"ZJBR"疗疾:exert dispel"ZJSEP"b8:内功"ZJBR"加力:enforce"ZJSEP"b9:暂未"ZJBR"设定:look"ZJSEP"b10:暂未"ZJBR"设定:look"ZJSEP"b11:乘雕"ZJBR"飞行:help mapb"+"\n");
	else if(arg=="fight")
	{
		str = ZJBTSET;
		for(i=1;i<11;i++)
	...
	else if(sscanf(arg,"reset %d",bt)==1)
	{
		...
		write(ZJOBLONG"你想设定按钮"HIG+chinese_number(bt)+NOR"为什么功能？\n"ZJOBACTS2+ZJMENUF(5,5,8,30)
			...
			+"金刚"ZJBR"不坏:mycmds reset "+bt+" 金刚"ZJBR"不坏：yong parry.jingang"ZJSEP
			+"乾坤"ZJBR"大挪移:mycmds reset "+bt+" 乾坤"ZJBR"大挪移：perform parry.nuoyi"ZJSEP
			+"斗转"ZJBR"星移:mycmds reset "+bt+" 斗转"ZJBR"星移：perform parry.yi"ZJSEP
			+"洛神"ZJBR"凌波:mycmds reset "+bt+" 洛神"ZJBR"凌波：perform dodge.lingbo"ZJSEP
			...
			+"吃护"ZJBR"心丹:mycmds reset "+bt+" 吃护"ZJBR"心丹：use huxin dan"
			+"\n");
	}
	return 1;
}
