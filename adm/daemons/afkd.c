...
void load_afks()
{
	...
	for(i=0;i<sizeof(files);i++)
	{
		...
		if (!file)
		{
			CHANNEL_D->do_channel( this_object(), "sys", "读取"+files[i]+"失败。");
			continue;
		}
...
void study_afk(object me)
{
	...
	if(me->query("afk"))
	{
		...
		{
			tell_object(me,"你的战斗经验需要达到" + exp + "才能研读"+book->query("name")+"学习"+to_chinese(name)+"。\n");
			me->delete("afk");
			return;
		}
...
void research_afk(object me)
{
	...
	if(me->query("afk"))
	{
		...
		{
			tell_object(me,"也许是缺乏实战经验，你感到难以继续研究" + to_chinese(skill) + "的问题了。\n");
			me->delete("afk");
			return;
		}
...
void derive_afk(object me)
{
	...
	if(me->query("afk"))
	{
		...
		if (! me->can_improve_skill(skill))
		{
			tell_object(me,"也许是缺乏实战经验，你感到难以继续提升" + to_chinese(skill) + "了。\n");
			me->delete("afk");
			return;
		}
	...
	if(me->query("afk"))
	{
		...
		if(sizeof(maxs=keys(max)))
		{
			for(i=0;i<sizeof(maxs);i++)
			{
				...
				{
					tell_object(me,"你的"+to_chinese(needs[i])+"超过"+max[maxs[i]]+"了,挂机中断。\n");
					me->delete("afk");
					return;
				}
			}
		}
		// tell_object(me,"你的"+query(file+"/name")+"开始挂机。\n");
		r = query(file+"/room");
		if(sizeof(rs=keys(r)))
		{
