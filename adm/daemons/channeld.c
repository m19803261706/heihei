...
mapping channels = ([
	...
	"rumor":([      "msg_speak": HIM "【传闻】%s：%s"NOR"\n",
...
varargs int do_channel(object me, string verb, string arg, int emote)
{
 ...
 int is_player;
	...
	if (userp(me) && verb == "rumor" && ! objectp(present("rumor generator", me)))
		...
		return notify_fail("你的精神不足以散播传闻！\n");
	...
	{
		if(userp(me) && VIP_D->query_daoshi(me->query("id")))
			svip = sprintf(BOLD HIY"%s导 "NOR,ZJSIZE(23));
		/*
		else if((v=me->query("zjvip/level"))>0 && !me->query("env/no_vip"))
		{
			if(v>9)
			{
				svip = sprintf(BOLD HIY"%s尊 "NOR,ZJSIZE(23));
			}
			else
			{
				v = 32-v;
				svip = sprintf(BOLD HIY"%sV%d "NOR,ZJSIZE(v),32-v);
			}
		}
		else */
		svip = "";
		...
		if (who == me->name(1))
			who = svip +ZJURL("cmds:look "+me->query("id"))+ZJSIZE(22)+ channels[verb]["msg_color"] + who+NOR;
