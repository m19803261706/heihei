...
//返回发放任务者列表
public mapping query_mlist() { return mlist; }
void create()
{
	seteuid(getuid());
	set("channel_id", "任务精灵");
...
private void skill_bonus(object me, object who)
{
	...
	if (who->query_skill(sk, 1) > 0 || !SKILL_D(sk)->valid_learn(who))
		return;
	if (me->query("family")) {
	...
	{
		tell_object(who, "可惜紫阳神功深奥之极，我再难有所感悟，看你资质不错，这本秘籍你拿去研究吧！”\n" );
...
无极仙丹 20%
	*/
	string *ob_list1 = ({
		...
		"/clone/vip/dan_str",
		"/clone/vip/dan_str",
		"/clone/vip/jiuzhuan1",//无极
		"/clone/vip/jiuzhuan1",//无极
		"/clone/vip/jiuzhuan1",//无极
		"/clone/vip/jiuzhuan1",//无极
	});
	
	string *ob_list3000 = ({//3000环必出
		"/d/item/obj/butian",
		"/d/item/obj/tiancs",
	});
	...
	//每周次数累计
	num = who->query("shimen/today");
	//每1000次清0
	num_t = who->query("quest_count");

	// 门客使用不同的计算方案
	if (who->query("is_menke"))
	{
		num = who->query("menke_reward/shimen/today");
	}
	...
	if (stringp(arg)) {
		...
		ob = new(ob_list3000[random(sizeof(ob_list3000))]);
		chn = 1;
	} else if (num_t>= 1000) {
		ob = new(ob_list1[random(sizeof(ob_list1))]);
		chn = 1;
	} else if (num_t>= 850) {
		ob = new(ob_list2[random(sizeof(ob_list2))]);
		chn = 1;
	} else if (num_t>= 650) {
		ob = new(ob_list3_4[random(sizeof(ob_list3_4))]);
	} else if (num_t>= 400) {
		ob = new(ob_list5[random(sizeof(ob_list5))]);
	} else if (num_t>= 250) {
		ob = new(ob_list6[random(sizeof(ob_list6))]);
	} else
	...
	ob->move(who, 1);
	log_file("job/shimen1", sprintf("%s：%s(%s)完成第%d个师门任务获得了%s。\n",ctime(time()),who->query("name"),who->query("id"),num_t,ob->query("name")));
...
//在原来的基础上再提高1倍 by luoyun 2018.6.4
        percent *=2;
...
if (who->query("relife/zhuanshi"))
	...
	pot *= 2;//潜能奖励再提升一倍 by luoyun 2018.6.27
	...
	//如果是门客，先记录奖励，等到门客任务结束再给,
	if (who->query("is_menke"))
	{
		who->add("menke_reward/combat_exp", exp);
		who->add("menke_reward/potential",pot);
		who->add("menke_reward/experience",mar);
		who->add("menke_reward/shen",shen);
		who->add("menke_reward/score",score);
		who->add("menke_reward/weiwang",weiwang);
		who->add("menke_reward/gongxian",1); 
		who->add("menke_reward/shimen/today",1);
	...
	else
	{
		...
		who->add("gongxian",1);
		who->add("shimen/today",1);

		if (who->query("shimen/today") > 1000)
		{
			who->add("bind_yuanbao",1);
			tell_object(who, "你本周已经完成了超过1000个师门任务，额外获得了一个绑定元宝。\n");
		}
	...
	if (who->query_condition("db_exp")) {
		who->add("combat_exp", exp);
		tell_object(who,"在天灵丹的作用下，你额外获得了"HIG+chinese_number(exp)+NOR"点经验。\n");
	}
	if (who->query_condition("db_exp2")) {
		who->add("combat_exp", exp*2);
		tell_object(who,"在玄灵丹的作用下，你额外获得了"HIG+chinese_number(exp*2)+NOR"点经验。\n");
	}
}
...
int ask_quest(object me, object who)
{
	...
	// 	letter->move(me);
	// 	message_vision("$N对$n道：“这封信你帮我交到" HIC + ob->name() + "("+ ob->query("id") + ")" +  NOR "手中，他现在应该在" + place +environment(ob)->query("short")+ "，我会安排车马送你前行，然后把回执带回来给我！”\n", me, who);

	// 	// 告诉玩家车马已经安排好了，可以直接去目的地
	// 	message_vision("$N对$n道：“车马已经安排好了，你可以直接去" + environment(ob)->query("short") + "了！”\n", me, who);
	// 	// 直接把玩家传送刀目的地
	// 	who->move(environment(ob));
	// 	// 车夫把玩家送到目的地，车夫告诉玩家把信送到哪里
	// 	tell_object(who, "你到了" + environment(ob)->query("short") + "，这里的车夫告诉你把信送到" + ob->name() + "手中。\n");
	...
	if (exp > 0) //取消送信任务
	{
		...
		if (!who->query("is_menke"))
		{
			...
			ob->add_temp("apply/armor", 10 + level * 12);
		}
		else
		{
			// 标记我是一个门客类型的师门怪物NPC
			ob->set_temp("is_menke_npc", 1);
			ob->set_temp("quest_menke_object",who);
		}

		// 标记任务对象
		who->set("quest/object", ob);
	...
	who->move(environment(ob));
	// 自动叫杀
	who->force_me("kill " + ob->query("id"));

	// 你疾行而去，不多时已经来到了目的地
	tell_object(who, "你疾行而去，不多时已经来到了" + environment(ob)->query("short") + "。\n");
...
int start_quest(object me, mapping quest)
{
	...
	else
	{
		// 标记我是一个门客类型的师门怪物NPC
		ob->set_temp("is_menke_npc", 1);
		ob->set_temp("quest_menke_object",me);
	}

	ob->set_temp("quester", me->query("id"));
	// 标记任务对象
	me->set("quest/object", ob);
	NPC_D->random_move(ob);
	NPC_D->random_move(ob);
	NPC_D->random_move(ob);
	place = ob->query("place");
	...
	if (level > 0 && random(10) == 1)
	{
		...
		//玩家眉毛一挑，道：“随说此事有点不对劲，但超不出掌控之外！”
		//xxx，岂能逃出我的手掌心！
		message_vision("$N眉毛一挑，道：“随说此事有点不对劲，但超不出掌控之外！”\n", me);
		//接着你运起轻功，飞身疾行而去，不一会儿就到了environment(ob)。
		tell_object(me, "接着你运起轻功，飞身疾行而去，不一会儿就到了" + environment(ob)->query("short") + "。\n");
		message_vision("$N对$n道：“" + ob->name() + "，你岂能逃出我的手掌心！”\n", me, ob);
	}
	...
	// 自动叫杀
	me->force_me("kill " + ob->query("id"));

	// 玩家对npc说:xxx总算找到你了，速速归天吧！
	message_vision("$N对$n说道：“" + ob->name() + "，速速归天吧！”\n", me, ob);
...
public void start_all_quest()
{
	...
	foreach (name in qlist)
	{
		...
		{
			// 没有这个任务
			log_file("nosave/quest",sprintf("%s invalid quest: %s\n",log_time(), name));
			continue;
		}
...
private void heart_beat()
{
	...
	foreach (key in keys(total))
	{
		...
		case QUEST_CREATE:
			...
			{
				// 创建任务超过有效时间了，结束之
				log_file("nosave/quest", sprintf("%s quest: %s create timeout\n%O",
