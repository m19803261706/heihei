...
public void map_user(object user)
{
	...
	if (wizardp(user) || user->query("id") == "luoyuna" || user->query("id") == "luoyunb" 
		||user->query("id") == "i3231752" || user->query("id") =="i3231757") {
		name = user->query("id");
...
public void add_shimen(object user)
{
	...
	// 如果我是门客，记录到主人身上
	if (user->query("master_id")) {
		name = user->query("master_id");
	}
...
void heart_beat()
{
	...
	*/
	...
	//tell_object(find_player("luoyun"),sprintf("local==%O\n", local));
	day = local[6];//星期
	min = local[0];//秒
	//if ((d == "Mon") &&(h == 0) &&(s == 0) && !ok) {
	if (day == 1 && !ok) {
