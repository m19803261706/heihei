...
varargs int skill_power(object ob, string skill, int usage, int delta)
{
 int  level, power;
	...
	//tell_object(find_player("luoyun"), sprintf("skill===%O   skill=%O\n", skill, ob->query_skill_mapped(skill)));
	//辟邪剑激发到相应技能上增强20%技能效果。by luoyun 2018.5.21
	if (ob->query_skill_mapped(skill) == "pixie-jian")
		level += level *20 /100;
...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
 ...
 string damage_info;
	...
	if (random(ap)+ap/2 < random(dp)+dp/3 && victim->query("qi")!=0)
	{  // Does the victim dodge this hit?
		...
		{
			if (random(ap)+ap/2 < random(dp)+dp/3 && random(my["int"]) > 14)
			{
	...
	else  //如果躲闪不成功，就判断是否增加攻击的奖励
	{
		...
		if (userp(me) || userp(victim))     
		{
			...
			if (random(ap)+ap/2 < random(dp)+dp/3 && random(my["int"]) > 12 )
			{
		...
		if (random(ap)+ap/2 < random(pp)+pp/3 && victim->query("qi")!=0)
		{
			//如果没有特殊招架，则给予 [基本招架]
			...
			//如果招架值小于攻击，而招架成功，并且计算 xx>30
		      if (userp(me) || userp(victim))  
		      {
				if (random(ap)+ap/2 < random(pp)+pp/3 && random(my["int"]) > 14)
				{
					//如上，通过就给予招架奖励
					if (your["combat_exp"] < EXP_LIMIT && victim->query("combat_exp")<me->query("combat_exp"))
					{
		...
		else
		{
			...
			while (damage > 0)
			{
				...
				{
					foo = SKILL_D(force_skill)->valid_fdamage(me, victim, damage, weapon);
					if (stringp(foo))
						damage_info += foo;
					else if (intp(foo))
						damage += foo;
				...
				if (stringp(parry_skill = victim->query_skill_mapped("parry")))
				{
					victim->set_temp("parry_valid_damage", 1);
					foo = SKILL_D(parry_skill)->valid_damage(me, victim, damage, weapon);
					victim->delete_temp("parry_valid_damage");
					if (stringp(foo))
						damage_info += foo;
					else if (intp(foo))
						damage += foo;
...
//暴击修正
	...
	if (damage > 0)
	{
		...
		if ((!me->query_temp("fight_team")) && (! me->is_killing(your["id"])) && (! victim->is_killing(my["id"])) 
			&& ! victim->query("not_living") && (your["qi"] * 3 <= your["max_qi"] || your["jing"] * 3 <= your["max_jing"]))
		{
			me->remove_enemy(victim);
			victim->remove_enemy(me);
...
varargs string do_damage(object me, object target, mixed type, int damage, int percent, mixed final)
{
	...
	damage += apply+me->query_gain_damage()/2;
	msg = "";
	foo_after_hit = 0;
	...
	while (damage > 0)
	{
		...
		if ((jiali = my["jiali"]) > 0 && my["neili"] >= jiali)
		{
			...
			{
				damage_bonus = jiali * damage / 350;
			}
			else
			{
				damage_bonus = jiali * damage / 500;
				skill = target->query_skill_mapped("force");
				if (stringp(skill))
				{
					if(!(foo = SKILL_D(skill)->valid_fdamage(me, target, damage_bonus)))
		...
		if (damage > 2000)	damage = (damage - 1600) / 3 + 1600;
		else if (damage > 1200)	damage = (damage - 1200) / 2 + 1200;
		...
		if (stringp(final))
		{
			...
			msg += final;
			if(damage>0)
				msg += HIG"$N对$n造成 "+damage+"/"+wound+" 点 "HIR+(damage_type?damage_type:"伤害")+NOR"\n";
...
varargs string do_respond(object me, object target, int damage, int percent, mixed final)
{
	...
	while (damage > 0)
	{
		...
		{
			msg += final;
			msg += HIG"$n对$N造成 "+damage+"/"+wound+" 点 "HIR+("反伤")+NOR"\n";
			msg += "( $N" + eff_status_msg(target->query("qi") * 100 / target->query("max_qi")) + ")\n";
		}
...
void auto_fight(object me, object obj, string type)
{
	...
	if (me->query_temp("owner") == obj || obj->quety_temp("owner") == me)
		return;
...
void start_berserk(object me, object obj)
{
	...
	if (me->is_fighting(obj) || environment(me) != environment(obj) || (environment(me)->query("no_fight") && !obj->query_condition("killer")))
		return;
...
void killer_reward(object killer, object victim)
{
	...
	if (playerp(victim)&&victim->query("combat_exp")<200000)
	{
		// clear punish info.
		...
		tell_object(victim,HIM + "新手保护期[<20万经验]免去了你的死亡惩罚" + NOR"\n");
	...
	else if (playerp(victim))
	{
		...
		if (objectp(killer) && playerp(killer) && playerp(victim))
		{
			...
			if (cname &&
			    ...
			    couple != killer &&
			    couple->query("nosave/marry") <= 1 &&
			    victim->query("nosave/marry") <= 1)
			{
				couple->add("nosave/sadly", 1);
...
int calc_wound(object attacker, object victim, int wound)
{
	...
	if (objectp(attacker) &&
	    ...
	    playerp(victim))
	{
		wound = (wound - victim->query_gain_armor() - victim->query_temp("apply/armor")/2) * PVP_WOUND_SCALE / 100;
	}
	else wound -= (victim->query_temp("apply/armor")/2 + victim->query_gain_armor());
