...
int fix_pay(int amount)
{
	...
	if (amount >= 50000)//一次性充值达到5千则按200%
		return amount * 2;
	...
	if ((today[4]+1)==5 && today[3] >= 25 || (today[4]+1)==6 && today[3] <= 8) {
		...
		if(amount>=10000) amount += amount/2;
		else if(amount>=5000) amount += amount*2/5;
		else if(amount>=2000) amount += amount*3/10;
		else if(amount>=1000) amount += amount/5;
