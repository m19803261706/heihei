...
void create()
{
	...
	message("channel", HIR"【官府】:"HIY"听说有飞贼在"+HIG+"测试地点"+ZJURL("cmds:goto 测试")+"飞过去"+NOR+HIY"一带出没，请留心自身财物安全！"NOR"\n", users());
	CHANNEL_D->do_channel( this_object(), "sys", "飞贼出现了。"+ZJURL("cmds:goto 测试")+"飞过去"+NOR"。");
	set_heart_beat(20);
}
...
void check_time()
{
	...
	if ((s == 50) && !ok) {
		if (!room = do_copy())
			...
			if (ob->move(room)) {
				message("channel", HIR"【官府】:"HIY"听说有飞贼在"+HIG+path_where+ZJURL("cmds:goto "+file_name(ob))+"飞过去"+NOR+HIY"一带出没，请留心自身财物安全！"NOR"\n", users());
