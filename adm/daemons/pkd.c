...
int join_competition(object ob)
{
	...
	if (state != SLEEPING)
		return notify_fail("现在竞技场正在举行活动，你还是等等再说吧。\n");
	...
	message_competition((ultrap(ob) ? "大宗师" : "") + ob->name(1) + "进入竞技场。");
...
private void change_state(int new_state)
{
	...
	case GET_READY:
		...
		ready_time = time();
		message_competition("听说一年一度的" + tlist[selected]["name"] + "马上就要举行了，不知道今年的竞技王是谁？");
		set_heart_beat(1);
		send_invite_message();
		break;
...
private int start_competition()
{
	...
	if (sizeof(obs) == 2)
	{
		...
		{
			message_vision("巫老大在外面喊道：“就你们夫妻俩个参加？还是算了吧。”\n", obs[0]);
			return 0;
		}
	}
	...
	set_heart_beat(1);
	message("vision", HIG "竞技比赛已经开始，快动手吧！\n", obs);
...
private void auto_check()
{
 ...
 int i,jifen;
	...
	if (sizeof(total) < 1)
	{
		msg = "听说竞技大赛参赛者死的死，逃的逃，现在一个人都没有啦！";
	} else
	if (sizeof(total) == 5)
	{
		jifen = 30*(selected+1);
	...
	if (sizeof(total) == 1)
	{
		...
		msg = "听说本次竞技大赛圆满结束，" + total[0]->name(1) + "成为竞技冠军！";
		give_bouns(total[0]);
		kickout_players();
	} else
		return;
...
*/
	...
	msg += HIG"获得竞技大赛冠军额外奖励物品"+ob->short()+NOR"。\n";
	ob->move(me);
...
private void send_invite_message()
{
	...
	message("vision", HIR "你收到了巫老大撒下的武林盟帖，邀请你参加这次" +
