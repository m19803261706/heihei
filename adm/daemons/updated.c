...
nosave mapping skill_list = ([
	...
	"zuoyou-hubo" 		: (["congenital" :(["con" :20, ]), ]),
	"qiankun-danuoyi" 	: (["congenital" :(["int" :22, ]), ]),
	"jiuyang-shengong" 	: (["congenital" :(["int" :25, "con" :29, ]), ]),
	"lonely-sword" 		: (["congenital" :(["int" :34, ]), "character" :({"狡黠多变", }), ]),
	"jiuyin-shengong" 		: (["congenital" :(["str" :22, ]), "character" :({"光明磊落", }), ]),
	"never-defeated" 		: (["congenital" :(["int" :22, ]), "character" :({"狡黠多变", }), ]),
	"bluesea-force" 		: (["congenital" :(["con" :22, ]), "character" :({"心狠手辣", }), ]),
	"kuihua-shengong" 	: (["congenital" :(["dex" :22, ]), "character" :({"阴险奸诈", }), "sex" :({"无性", }), ]),
...
void check_family(object ob)
{
	...
	if (lvl = ob->query_skill("jingang-quan", 1)) {//非少林派弟子不可拥有
		if ( family_name !="少林派") {
			skname = "jingang-quan";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
		}
	}
	if (lvl = ob->query_skill("zui-quan", 1)) {//非少林派弟子不可拥有
		if ( family_name !="少林派") {
			skname = "zui-quan";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
	...
	if (lvl = ob->query_skill("count", 1)) {//非少林派弟子不可拥有
		if ( family_name !="桃花岛") {
			skname = "count";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
	...
	if (lvl = ob->query_skill("taohua-yaoli", 1)) {//非桃花岛弟子不可拥有
		if ( family_name !="桃花岛") {
			skname = "taohua-yaoli";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
	...
	if (lvl = ob->query_skill("qimen-wuxing", 1)) {//非桃花岛弟子不可拥有
		if ( family_name !="桃花岛") {
			skname = "qimen-wuxing";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
	...
	if (lvl = ob->query_skill("buddhism", 1)) {//非少林、非段家弟子，禅宗最多保留180级
		if (lvl >= 180
			&& family_name !="少林派"
			&& family_name !="段氏皇族"
			) {
			skname = "buddhism";
			ob->set_skill(skname, 180);
			log_delsk(ob, "set", skname, lvl, family_name+"门派不符");
		}
	}
	if (lvl = ob->query_skill("poison", 1)) {//非星宿、非欧阳、非胡家弟子不可拥有
		if ( family_name !="星宿派"
			&& family_name !="欧阳世家"
			&& family_name !="关外胡家"
			) {
	...
	if (lvl = ob->query_skill("xiaoyao-qixue", 1)) {//非逍遥弟子不可拥有
		if ( family_name !="逍遥派") {
			skname = "xiaoyao-qixue";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
		}
	}
	if (lvl = ob->query_skill("zuoyou-hubo", 1)) {//非全真、非古墓弟子不可拥有，无性之人最多学习50级
		if ( family_name !="全真教" && family_name !="古墓派") {
			skname = "zuoyou-hubo";
			ob->delete_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
		} else if (ob->query("gender") == "无性" && lvl >50){
			skname = "zuoyou-hubo";
			ob->set_skill(skname, 50);
			log_delsk(ob, "set", skname, lvl, family_name+"无性>50级");
		}
		if (ob->query("int")> 28) {
			skname = "zuoyou-hubo";
			ob->del_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"先天不符int>28");
		}
	}
	if (lvl = ob->query_skill("taoism", 1)) {//无性之人最多学习50级
		if (ob->query("gender") == "无性" && lvl >50){
			skname = "taoism";
			ob->set_skill(skname, 50);
			log_delsk(ob, "set", skname, lvl, family_name+"无性>50级");
		}
	}
	if (lvl = ob->query_skill("lamaism", 1)) {//非密宗弟子（大轮寺，血刀门），非段家弟子不可拥有
		if ( family_name !="血刀门"
			&& family_name !="雪山寺"
			&& family_name !="段氏皇族"
			) {
	...
	if (lvl = ob->query_skill("dispel-poison", 1)) {//非胡家弟子不可拥有
		if ( family_name !="关外胡家") {
			skname = "dispel-poison";
			ob->del_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
		}
	}
	if (lvl = ob->query_skill("yaowang-miaoshu", 1)) {//非胡家弟子不可拥有
		if ( family_name !="关外胡家") {
			skname = "yaowang-miaoshu";
			ob->del_skill(skname);
			log_delsk(ob, "del", skname, lvl, family_name+"门派不符");
		}
	}
...
void check_user(object ob)
{
	...
	if (!wizardp(ob) 
		&& ob->query("id")!="i3231752" 
	&& ob->query("id")!="i3231757" 
	&& ob->query("id")!="luoyunb" 
	...
	if (ob->query("couple") && ! ob->query("protected/marry"))
		ob->set("nosave/marry", 1);
...
void born_player(object me)
{
	...
	if (sizeof(files))
	{
		...
		msg += TALNET_MASTER->level_name(me,special);
		...
		if ((sizeof(files) && random(30) == 1 && me->query("combat_exp") > 1)|| me->query_temp("born_skill2"))
		{
			...
			special = files[random(sizeof(files))];
			me->set("special_skill/" + special, 1);
			msg += HIG "、" NOR + TALNET_MASTER->level_name(me,special);
			files -= ({ special });
