...
//新增数据，用于计算玩家在某一阶段是否没有受到任何伤害
int time_damage = 0;
//是否开启受伤计算
int is_on_damage = 0; //默认关闭

//新增函数用于关闭和开启受伤计算 by baiyi
void set_on_damage(int flag)
{
	is_on_damage = flag;
...
varargs int receive_damage(string type, int damage, object who)
{
	...
	// 采取一次拦截，查看last_damage_from是否是门客，如果是门客，那么last_damage_from就是门客的主公
	if (objectp(who) && who->query("is_menke"))
	{
		mixed owner;
		owner = find_player(who->query("master_id"));

		if (objectp(owner)) //如果主公在线那么计算造成伤害者为主公，否则仍然是门客本身
		{
			// 同时满足第二个条件，我并不在执行任务
			if (!who->query("doing_quest"))
			{
				last_damage_from = owner;
				last_damage_name = owner->name(1);
			}
		}
	}
...
varargs void die(object killer)
{
 ...
 int time;
	...
	if(me->query("combat_exp")>=200000&&objectp(kuilei=present("tishen kuilei",me)))
	{
		kuilei->add_amount(-1);
