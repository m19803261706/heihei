...
void create()
{
	set_name(HIW "《葵阳宝录2》" NOR, ({ "kuihua baodian2", "baodian2", "book2", "kuihua2"}));
	set_weight(600);
	if (clonep())
		set_default_object(__FILE__);
	else {
		set("unit", "本");
		set("long", HIR "这就是武林第一秘籍《葵阳宝录2》，相传是一位宫中太监所作的手抄真迹，你可以"
...
int do_du(string arg)
{
 ...
 int plvl, con;
	...
	{
		write("自古没有听说女人可以读《葵阳宝录2》的。\n");
		return 1;
	}

	if ((string)me->query("gender") == "男性")
	{
	       write("《葵阳宝录2》的第一页写着：欲练神功，必先自宫。\n");
	       return 1;
	}

	if (me->query_skill("pixie-jian", 1))
	{
		write("你不散掉胜邪剑法没办法研读这本书。\n");
		return 1;
	}
	...
	if (me->query_skill("xixing-dafa", 1) > 1 || me->query_skill("tianmo-jian", 1) > 1) {
		tell_object(me, "你已经学会吸功大法或是天魔剑法，这本书已经完全看不懂了。\n");
		return 1;
	}
	if (me->query("family/master_id") == "ren woxing") {
		tell_object(me, "你师傅任横行最讨厌的就是东方老贼，学这种武功不太合适吧？\n");
		return 1;
	}
