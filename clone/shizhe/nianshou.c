...
void get_damage(object me, int damage, int type)
{
	...
	if("/adm/daemons/newbossd"->query_qilin()<3 && damage>200 && damage > random(7500))
	{
		obj = new("/clone/newyear/nianshou");
		CHANNEL_D->do_channel(ob,"rumor", me->query("name")+"重创年兽获得"+obj->short()+"。");
		log_ufile(me,"qilin","重创("+damage+")年兽获得"+obj->short()+"。\n");
		tell_object(me,"你重创年兽获得"+obj->short()+"。\n");
		obj->move(me);
		"/adm/daemons/newbossd"->set_qilin("/adm/daemons/newbossd"->query_qilin()+1);
		"/adm/daemons/newbossd"->save();
	}
