...
void update_age()
{
	...
	//lvs = ({ 300, 900, 1600, 2500, 3600, 4800, 6200, 8000, 10000, 999999999 });
	...
	{
		all = query("zjvip/all_pay")+query("zjvip/sign_exp") + query("guanggao/total");
...
void user_dump(int type)
{
	...
	if (query_temp("pending/auto_dazuo")) return;//离线打坐
	if (query_temp("pending/auto_research")) return;//离线研究
	if (query_temp("pending/auto_lian")) return;//离线练习
	if (query_temp("pending/auto_learn")) return;//离线学习
	switch(type)
	{
	case DUMP_NET_DEAD:
		if (environment())
...
private void net_dead()
{
	...
	if (!query("doing")&&!(environment(this_object())&&environment(this_object())->query("close_room")||query("afk")) &&!query_temp("pending/auto_dazuo")&&!query_temp("auto_research") && !query_temp("auto_lian") && !query_temp("auto_learn"))
		set_heart_beat(0);
