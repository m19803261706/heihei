...
// 1.
	...
	message("vision", "\n杨霄走了过来，跟" + monk->query("name") + "俯耳轻声说了几句。\n", room, monk);
...
int waiting(object me)
{
	...
	{
		if( random(10) == 0 ) 
		say("\n杨霄说道：" + RANK_D->query_respect(fighter) + "去请了范右使没有？ 照理他现在该到了罢？\n");
...
int do_recruit(object me, object fighter)
{
 mapping ft_fam;
	...
	{
		command("bow");
		say("杨霄说道：" + RANK_D->query_respect(fighter) + "此番过阵，当对本身修行大有助益，百尺竿头，更进一步，杨霄就此别过！\n");
		call_out("do_back", 5, me );
	}
	else
	{
		say("\n杨霄说道：恭喜你闯过了光明圣火阵！本教有此等杰出弟子，届时必当为武林放一异彩！\n");
		call_out("do_back", 5, me );
	}
	return 1;
}
