...
{
	seteuid(getuid());
	set_name("欧阳轲", ({ "ouyang ke", "ouyang", "ke" }));
	...
	set("chat_chance", 2);
	set("chat_msg", ({
		"欧阳轲唱道：漂亮姑娘你请留步￣￣让我多看你一眼￣￣\n",
		"欧阳轲色迷迷地看着街边的小姑娘。\n",
		"欧阳轲把纸扇一展，转过身去，深情地吟道：红颜知己最难得￣\n",
	}) );
...
void greeting(object ob)
{
	...
	if ((string)ob->query("gender")=="女性")
	{
	   ...
	   say("欧阳轲叹了一口气说：这位"+RANK_D->query_respect(ob)+
	       "，你要是能给我弄一个女人来就好了。\n");
	return;
}
...
void attempt_apprentice(object ob)
{
	...
	{
		message_vision("欧阳轲动情地说道：这位" + RANK_D->query_respect(ob) +
...
int accept_object(object who, object ob)
{
	...
	if (environment(this_object())->query("short") == "观乐楼")
	{
	...
	if (userp(ob))
	{
		...
		{
			command("say 靠！这种女人人尽可夫，我欧阳轲可不想要。");
			return 0;
		}
