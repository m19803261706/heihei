...
void die(object killer)
{
	...
	while (1)
	{
		...
		if (!objectp(dob))
		{
			if (!query_temp("is_menke_npc"))
			{
				dob = UPDATE_D->global_find_player(query_temp("quester"));
			}
			else
			{
				dob = query_temp("quest_menke_object");
			}
		}

		// 记录：此任务已经终止，非门客执行的任务检查任务对象
		if (stringp(quester = query_temp("quester")) && !query_temp("is_menke_npc"))
		{
			// 领取该任务的人不一定是玩家
		...
		else if (query_temp("is_menke_npc") && stringp(quester = query_temp("quester")))//非玩家默认为门客
		{
			qob = find_living(query_temp("quester"));

			// 如果qob为空，从当前地图上找
			if (!qob)
			{
				qob = present(query_temp("quester"), environment(this_object()));
			}
		...
		log_file("nosave/killed_die", sprintf("%s %s(%s) died at %s because %s.\n",
	    ...
	    ! mapp(quest = QUEST_D->prepare_quest(qob)))
	{
		if (dob->query("is_menke"))
		{
			object player = find_player(dob->query("master_id"));
	
			if (objectp(player))
			{
				tell_object(player,"门客没有找到任务所有者，任务失效\n");
			}
