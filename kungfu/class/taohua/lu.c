...
void create()
{
	...
	set_skill("luoying-shenzhang", 160);// 落红凝剑掌
...
void attempt_apprentice(object ob)
{
	...
	if (ob->query("family/master_name") == "黄岛主" &&
...
int accept_object(object who, object ob)
{
	...
	if (strsrch(ob->query("name"), "完颜康的头颅") < 0 &&
	    strsrch(ob->query("name"), "完颜鸿烈的头颅") < 0 &&
	    strsrch(ob->query("name"), "欧阳锋的头颅") < 0 &&
	    strsrch(ob->query("name"), "欧阳轲的头颅") < 0 &&
	    strsrch(ob->query("name"), "彭炼虎的头颅") < 0 &&
	    strsrch(ob->query("name"), "梁梓翁的头颅") < 0 &&
	    strsrch(ob->query("name"), "灵芝上人的头颅") < 0 &&
	    strsrch(ob->query("name"), "沙横天的头颅") < 0 &&
	    strsrch(ob->query("name"), "侯同海的头颅") < 0 &&
	    strsrch(ob->query("name"), "沈青刚的头颅") < 0 &&
	    strsrch(ob->query("name"), "吴擎烈的头颅") < 0 &&
	    strsrch(ob->query("name"), "马青雄的头颅") < 0 &&
	    strsrch(ob->query("name"), "钱青剑的头颅") < 0 &&
	    strsrch(ob->query("name"), "大金兵的头颅") < 0 &&
	    strsrch(ob->query("name"), "段齐德的头颅") < 0) 
	{
