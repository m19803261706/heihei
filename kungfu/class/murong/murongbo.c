...
void create()
{
	set_name("慕容柏", ({ "murong bo", "murong", "bo" }));
	set("long", @LONG
...
void attempt_apprentice(object ob)
{
	...
	if((int)ob->query("combat_exp") < 200000)
	{
		...
		command("say 你的经验还不够，先和我儿慕容负学习吧！");
		return ;
	}
	...
	message_vision("慕容柏大喜：我们慕容家真是英才迭出，复国有望啊！\n",
		       this_object());
	return;
}
...
mixed ask_me()
{
	...
	command("say 这其中的奥妙乃是用巧妙的招式配合我"
		"慕容家的紫徽心法，你要好好体会，去吧！");
	tell_object(me, HIG "你学会了移星换斗。"NOR"\n");
...
string ask_book2()
{
	...
	if (me->query_skill("douzhuan-xingyi", 1) < 100)
		return "你的移星换斗练的还不到家，我还不能给你这本秘籍。";
