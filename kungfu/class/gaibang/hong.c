...
void create()
{
	set_name("洪九指", ({"hong qigong", "hong", "qigong"}));
	set("nickname", "九指神丐");
	set("gender", "男性");
	set("age", 75);
	set("long", 
		"他就是丐帮第十七任帮主，号称“北丐”的洪九指老爷子。\n");
	...
	set("chat_chance", 1);
	set("chat_msg", ({
		"洪九指叹了口气道：“唉……何时能再吃到蓉儿做的「叫化鸡」啊……”\n",
		"洪九指说道：“俗话说「吃在江南」，所以老叫化特地跑来扬州看看。”\n",
		"洪九指喃喃道：“我们丐帮的「镇岳锁龙劲」是天下最霸道的掌法。”\n",
	}));
...
int accept_object(object who, object ob)
{
	...
	if (who->query("protected/marry") > 1)
	{
	...
	tell_object(who, HIG + me->name() + HIG "答应传授" +
			 hob->name(1) + "镇岳锁龙劲法和基本杖法作为报答。"NOR"\n");
	tell_object(who, HIG + me->name() + HIG "答应传授" +
...
int try_to_learn_db()
{
	...
	{
		command("say 也不看看你是什么辈分，居然想学神丐棒法？");
		return -1;
	}
	...
	command("sigh");
	command("say 神丐棒法本来只传历代帮主，但是现在江湖多争乱，你"
		"也是本帮高手，却也学之无妨。");
...
int try_to_learn_ds()
{
	...
	{
		command("say 你要学镇岳锁龙劲？哈哈，等你升到本帮长老再说吧！");
		return -1;
	}
...
void attempt_apprentice(object ob)
{
	...
	if (ob->query_str() < 40 && ob->query_int() < 42 && ob->query_con() < 40)
	{
	    ...
	    tell_object(ob, "洪九指收徒要求并不高，后天臂力或者根骨任意一项达到40，亦或悟性达到42。\n");
		return;
	}
