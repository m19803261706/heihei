...
void create()
{
	set_name("胡飞", ({"hu fei", "hu", "fei"}));
	set("gender", "男性");
	set("nickname", "雪山飞"HIG"狐"NOR);
	set("age", 25);
	set("long", @LONG
他就是胡亿刀之子，因其武功高强神出鬼没。在江湖上人送外号「雪山飞狐」。
...
LONG);
	...
	set("chat_chance", 10);
	set("chat_msg", ({
		"胡飞叹了口气道：“不知什么时候还能再见到若兰姑娘。\n",
		"胡飞切齿恨道：“阎基老贼暗中下毒，我必手刃此獠！”\n",
	}));
	...
	set_skill("hujia-daofa", 240);      // 飞狐刀法
...
string ask_hujia_book()
{
	...
	if ((int)me->query_skill("hujia-daofa", 1) < 40)
		return "你先在飞狐刀法上好好下下功夫，然后再研读刀谱总诀不迟。\n";
...
void attempt_apprentice(object ob)
{
	...
	me = this_object();
	if (ob->name(1) == "胡亿刀" || ob->name(1) == "胡壹刀")
	{
