	...
	if (ap / 2 + random(ap * 2) > dp)
	{
		...
		if (target->is_busy() || ap / 2 + random(ap) > dp)
		{
			msg += HIY "$n" HIY "见来招即巧又拙，不同于人间"
			       ...
			       HIY "见状连出" + chinese_number(n) + "招！"NOR"\n";
			message_combatd(msg, me, target);
			me->start_busy(1 + random(n));
			while (target && n-- && me->is_fighting(target))
			{
			...
			if (weapon2 && random(ap) / 2 > dp && weapon2->query("type") != "pin")
			{
				...
				if(weapon2->query("user_make"))
					weapon2->unequip();
				else
					weapon2->move(environment(me));
			} else
			{
				msg = HIY "$n竭力抵挡，一时间再也无力反击。"NOR"\n";
			}
		} else
		{
			msg += HIY "$n" HIY "只办了个勉力遮挡，全然无法反击。"NOR"\n";
			me->start_busy(1);
