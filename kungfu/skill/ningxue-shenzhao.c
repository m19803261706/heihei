...
int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
	    return notify_fail("练败血伸爪必须空手。\n");

	if ((int)me->query("max_neili") < 800)
	    return notify_fail("你的内力太弱，无法练败血伸爪。\n");

	if ((int)me->query_str() < 40)
	    return notify_fail("你现在膂力孱弱，还无法练败血伸爪。\n");

	if ((int)me->query_dex() < 40)
	    return notify_fail("你现在身法欠佳，还无法练败血伸爪。\n");

	if ((int)me->query_skill("force") < 200)
	    return notify_fail("你的内功太差，无法练败血伸爪。\n");

	if ((int)me->query_skill("claw", 1) < 180)
	    return notify_fail("你的基本爪法太差，无法领会败血伸爪。\n");

	if ((int)me->query_skill("claw", 1) < (int)me->query_skill("ningxue-shenzhao", 1))
	    return notify_fail("你的基本爪法火候不够，无法领会更高深的败血伸爪。\n");
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < 350)
		return notify_fail("你的内力不够练败血伸爪。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	{
		return HIR "只听$n" HIR "一声惨嚎，看来受到了败血伸爪毒劲的干扰！"NOR"\n";
	}
}
