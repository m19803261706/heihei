...
int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练折花错拳必须空手。\n");

	if (me->query("max_neili") < 1500)
		return notify_fail("你的内力修为远远不足，难以练习折花错拳。\n");

	if ((int)me->query_skill("force", 1) < 150)
		return notify_fail("你的内功火候尚浅，无法学习折花错拳。\n");

	if ((int)me->query("str") < 28)
		return notify_fail("你先天臂力不足，无法学习折花错拳。\n");

	if ((int)me->query("int") < 28)
		return notify_fail("你先天悟性不足，无法学习折花错拳。\n");

	if ((int)me->query_skill("unarmed", 1) < 200)
		return notify_fail("你的拳脚根基有限，无法体会折花错拳要诣。\n");

	if ((int)me->query_skill("cuff", 1) < 150)
		return notify_fail("你的拳法根基有限，无法体会折花错拳要诣。\n");

	if ((int)me->query_skill("hand", 1) < 150)
		return notify_fail("你的手法根基有限，无法体会折花错拳要诣。\n");

	if ((int)me->query_skill("claw", 1) < 150)
		return notify_fail("你的爪法根基有限，无法体会折花错拳要诣。\n");

	if ((int)me->query_skill("strike") < 150)
		return notify_fail("你的掌法根基有限，无法体会折花错拳要诣。\n");

	if ((int)me->query_skill("unarmed", 1) < (int)me->query_skill("baihua-quan", 1))
		return notify_fail("你的拳脚根基火候不足，无法领会更高深的折花错拳。\n");

	if ((int)me->query_skill("cuff", 1) < (int)me->query_skill("baihua-quan", 1))
		return notify_fail("你的拳法根基火候不足，无法领会更高深的折花错拳。\n");

	if ((int)me->query_skill("hand", 1) < (int)me->query_skill("baihua-quan", 1))
		return notify_fail("你的手法根基火候不足，无法领会更高深的折花错拳。\n");

	if ((int)me->query_skill("claw", 1) < (int)me->query_skill("baihua-quan", 1))
		return notify_fail("你的爪法根基火候不足，无法领会更高深的折花错拳。\n");

	if ((int)me->query_skill("strike", 1) < (int)me->query_skill("baihua-quan", 1))
		return notify_fail("你的掌法根基火候不足，无法领会更高深的折花错拳。\n");
