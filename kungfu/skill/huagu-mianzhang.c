...
int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练销骨绵掌必须空手。\n");

	if ((int)me->query_skill("force") < 40)
		return notify_fail("你的内功心法火候不够，练销骨绵掌会走火入魔。\n");

	if ((int)me->query("max_neili") < 250)
		return notify_fail("你的内力太弱，无法练销骨绵掌。\n");

	if ((int)me->query_skill("strike", 1) < (int)me->query_skill("huagu-mianzhang", 1))
		return notify_fail("你的基本掌法火候有限，无法领会更高深的销骨绵掌。\n");
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < 60)
		return notify_fail("你的内力不够练销骨绵掌。\n");
