...
int perform(object me, object target)
{
	...
	if (me->query_skill("piaoxue-zhang", 1) < 150) 
		return notify_fail("你的寒雪断云掌修为不够，无法施展" YUN "。\n");
	...
	if (me->query_skill_mapped("strike") != "piaoxue-zhang") 
		return notify_fail("你没有激发寒雪断云掌，无法施展" YUN "。\n");

	if (me->query_skill_prepared("strike") != "piaoxue-zhang")
		return notify_fail("你没有准备使用寒雪断云掌，无法施展" YUN "。\n");

	if (! living(target))
	       return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIW "$N" HIW "一声暴喝，陡然施出寒雪断云掌绝技「云海明灯」，瞬"
	      "间连续攻出数招。"NOR"\n";
