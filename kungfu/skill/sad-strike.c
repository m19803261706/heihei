...
int valid_learn(object me)
{
	...
	if (me->query("protected/marry") > 1)
		return notify_fail("你感情早已不纯，哪里还能领略到那种黯然销魂的感觉？\n");

	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练六阴殛神掌必须空手。\n");

	if ((int)me->query("max_neili") < 2000)
		return notify_fail("你的内力太弱，无法练六阴殛神掌。\n");

	if ((int)me->query_str() < 45)
		return notify_fail("你的膂力太弱，无法练六阴殛神掌。\n");

	if ((int)me->query_skill("unarmed", 1) < 150)
		return notify_fail("你的基本拳脚不够娴熟，无法练六阴殛神掌。\n");

	if ((int)me->query_skill("unarmed", 1) < (int)me->query_skill("sad-strike", 1))
		return notify_fail("你的基本拳脚水平有限，无法领会更高深的六阴殛神掌。\n");
