...
int valid_learn(object me)
{
	if ((int)me->query("max_neili") < 700)
		return notify_fail("你的内力不够，没有办法练腾蛇剑法。\n");

	if ((int)me->query_skill("force") < 90)
		return notify_fail("你的内功火候不够，没有办法练腾蛇剑法。\n");

	if ((int)me->query_skill("sword", 1) < (int)me->query_skill("jinshe-jian", 1))
		return notify_fail("你的基本剑法水平有限，无法领会更高深的腾蛇剑法。\n");
...
int practice_skill(object me)
{
	...
	if ((int)me->query("qi") < 70)
		return notify_fail("你的气不够，没有办法练习腾蛇剑法。\n");

	if ((int)me->query("neili") < 60)
		return notify_fail("你的内力不够，没有办法练习腾蛇剑法。\n");
