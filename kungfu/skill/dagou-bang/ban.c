...
int perform(object me, object target)
{
	...
	if (! target || ! me->is_fighting(target))
		return notify_fail("神丐棒法「绊」字诀只能对战斗中的对手使用。\n");
	...
	if ((int)me->query_skill("dagou-bang", 1) < 100)
		return notify_fail("你的神丐棒法不够娴熟，不会使用「绊」字诀。\n");
	...
	if (me->query_skill_mapped("staff") != "dagou-bang")
		return notify_fail("你没有激发神丐棒法，不能使用「绊」字诀。\n");

       if (! living(target))
	      return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIG "$N" HIG "使出神丐棒法「绊」字诀，手中的" + weapon->name() +
	      HIG "左封右逼，霎时连出数招。"NOR"\n";

	message_combatd(msg, me);
	me->add("neili", -180);
	count = me->query("int") * 3;
