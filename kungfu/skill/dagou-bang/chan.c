...
int perform(object me, object target)
{
	...
	if (! target || ! me->is_fighting(target))
		return notify_fail("神丐棒法「缠」字诀只能对战斗中的对手使用。\n");
	...
	if ((int)me->query_skill("dagou-bang", 1) < 70)
		return notify_fail("你的神丐棒法不够娴熟，不会使用「缠」字诀。\n");
	...
	if (me->query_skill_mapped("staff") != "dagou-bang")
		return notify_fail("你没有激发神丐棒法，不能使用「缠」字诀。\n");

       if (! living(target))
	      return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIG "$N" HIG "使出神丐棒法「缠」字诀，棒头在地"
	...
	if (ap / 2 + random(ap) > dp)
	{
		...
		target->start_busy((int)me->query_skill("dagou-bang",1) / 20 + 2);
	} else
	{
