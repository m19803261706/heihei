...
string query_txt()
{
	return "要求：折花错拳200级，拳脚、拳法、掌法、爪法、手法全部激发为折花错拳，折花错拳等级不能高于5种空手基本，内力大于1000"ZJBR
...
int perform(object me, object target)
{
	...
	if (me->query_skill_prepared("unarmed") != "baihua-quan" &&
		...
		return notify_fail("你现在没有准备使用折花错拳，无法施展百花错乱！\n");
	...
	if ((lvl = me->query_skill("baihua-quan", 1)) < 200)
		return notify_fail("你的折花错拳还不够纯熟！\n");

    if (! living(target))
	      return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIW "只见$N使出折花错拳的精妙百花错乱，擒拿手中夹着鹰爪功，左手查拳，右手绵掌。攻\n"
