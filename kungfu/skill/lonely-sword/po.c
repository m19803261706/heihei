...
int perform(object me, object target)
{
 ...
 int skill, ap, dp, ss;
	...
	if (! me->is_fighting(target))
		return notify_fail("九剑归一只能对战斗中的对手使用。\n");
	
	if (userp(me) && me->query("family/family_name") != "华山派" &&
	    me->query("family/family_name") != "华山剑宗")
		return notify_fail("你不是华山弟子，无法使用破兵式。\n");
	
	if (skill < 50)
		return notify_fail("你的九剑归一等级不够，练好了再来！\n");
	...
	if (ap / 2 + random(ap) > dp)
	{
		msg = HIC "$N" HIC "随意挥洒手中的" + weapon->name() + HIC
		      "，招招从出其不意的方位直指$n" HIC + to_chinese(type)[4..<1] + "中的破绽。"NOR"\n";
		n = 3 + random(5);
