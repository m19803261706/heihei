...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("sanyin-wugongzhao", 1) < 80)
		return notify_fail("你的六阴蜈蚣爪不够娴熟，无法使用「三阴毒爪」。\n");

	if (me->query_skill_mapped("claw") != "sanyin-wugongzhao")
		return notify_fail("你没有激发六阴蜈蚣爪，无法使用「三阴毒爪」。\n");
	...
	ap = me->query_skill("claw")+me->query_skill("poison", 1);
	dp = target->query_skill("parry")+me->query_skill("poison", 1);
	if (ap / 2 + random(ap) > dp)
	{
		damage = ap / 2 + random(ap / 2);
		msg += COMBAT_D->do_damage(me, target, UNARMED_ATTACK, damage, 50, (: final, me, target :));
		me->start_busy(2);
	} else
	{
