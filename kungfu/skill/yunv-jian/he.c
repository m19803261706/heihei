...
int perform(object me, object target)
{
	...
	if (me->query_skill("zuoyou-hubo", 1) < 150)
		return notify_fail("你的玄虚互质之术尚未修炼到家，难以分使这两种剑招。\n");

	if ((int)me->query_skill("yunv-jian", 1) < 120)
		return notify_fail("你的素女剑法不够娴熟，不能使用双剑合璧。\n");
	...
	if (me->query_skill_mapped("sword") != "yunv-jian")
		return notify_fail("你没有激发素女剑法，不能使用双剑合璧。\n");

       if (! living(target))
	      return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIY "$N" HIY "左手以全真剑法剑意，右手化素女剑法剑"
	      "招，双剑合璧同时刺出。"NOR"\n";
