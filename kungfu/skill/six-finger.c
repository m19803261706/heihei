...
mapping *action = ({
([      "name"  : "少商剑",
	"action": "$N反过手来，双手拇指同时捺出，嗤嗤两声急响，“" HIW "少商剑" NOR "”有如石迫天惊、风雨大至之势，指向$n的$l",
	"force" : 390,
	"attack": 160,
	"dodge" : 90,
	"parry" : 110,
...
int get_finish(object me)
{
	...
	{
		tell_object(me, "你努力想将六剑合一，未身在段氏皇族，怎么也无法体会其中精髓。\n");
		return 0;
	}
	if (me->query("int") < 32)
	{
	...
	{
		tell_object(me, "你觉得六极神剑极其深奥，看来多研究一下学问可能更有帮助。\n");
		return 0;
	}
	...
	if (random(10) < 7)
	{
		tell_object(me, "你觉得有所感悟，或许再演练一次就能融会贯通，练成六极神剑。\n");
		return 0;
	}
...
int valid_learn(object me)
{
	if (me->query("family/family_name") !="段氏皇族")
	{
		tell_object(me, "你练了半天才发现，未身在段氏皇族，怎么也不能进步分毫。\n");
		return 0;
	}
	
	if (me->query("int") < 32)
		return notify_fail("你研究了半天，也没能理解六极神剑的奥妙。\n");

	if (me->query("con") < 26)
		return notify_fail("你研究了一会儿，只觉得眼前金星乱冒，太阳穴突突的跳。\n");

	if (me->query_skill("literate", 1) < 200)
		return notify_fail("你觉得六极神剑极其深奥，不是你这种学问水平所能研究的。\n");
	...
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练六极神剑必须空手。\n");

	if ((int)me->query("max_neili") < 3000)
		return notify_fail("你的内力太弱，无法学六极神剑。\n");
	...
	if ((int)me->query_skill("finger", 1) < (int)me->query_skill("six-finger", 1))
		return notify_fail("你的基本指法水平有限，无法无法领会更高深的六极神剑。\n");
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < cost)
		return notify_fail("你的内力不够练六极神剑。\n");
